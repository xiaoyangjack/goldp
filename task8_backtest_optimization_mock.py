...
def generate_mock_data(start_date, end_date):
    ...
    dxy_prices = [dxy_base]
    for i in range(1, n):
        daily_change = np.random.normal(-0.0005, 0.002)  # DXY与黄金负相关
        new_dxy = dxy_prices[-1] * (1 + daily_change)
        dxy_prices.append(new_dxy)
    data['dxy_close'] = dxy_prices
...
def backtest_strategy(data, signals, atr_multiplier=1.5, fees=0.001, slippage=0.0005):
    """
    简单回测策略
    """
    df = data.copy()
    ...
    for i in range(1, len(df)):
        ...
        if position != 0:
            if position > 0 and current_price <= stop_loss:
                ...
                trade_return = (exit_price - entry_price) / entry_price - fees - slippage
                ...
                # 空头止损
                exit_price = stop_loss
                trade_return = (entry_price - exit_price) / entry_price - fees - slippage
    ...
    for i in range(1, len(df)):
        if position != 0:
            if position > 0:
                ...
                daily_return = (df['close'].iloc[i-1] - df['close'].iloc[i]) / df['close'].iloc[i-1]
            cumulative_returns.append(cumulative_returns[-1] * (1 + daily_return * abs(position)))
        else:
    ...
    avg_loss = np.mean([abs(t['return']) for t in loss_trades]) if loss_trades else 1
    ...
    for atr_multiplier in param_grid['atr_multiplier']:
        for batch_buy_min in param_grid['batch_buy_range_min']:
            for batch_buy_max in param_grid['batch_buy_range_max']:
                ...
                # 回测
                result = backtest_strategy(data, signals['signal'], atr_multiplier)
...
def compare_weight_adjustment(data):
    ...
    original_result = backtest_strategy(data, original_factor_data['signal'])
    
    # 使用新权重回测
    new_strategy = GRAMFourFactorStrategy(new_weights)
    new_factor_data = new_strategy.calculate_gram_factors(data)
    new_result = backtest_strategy(data, new_factor_data['signal'])
    
    # 打印对比结果
    print("\n权重调整前后对比:")
    print("原始权重 (各25%):")
    print(f"  总收益率: {original_result['total_return'] * 100:.2f}%")
    print(f"  年化收益率: {original_result['annualized_return'] * 100:.2f}%")
    print(f"  最大回撤: {original_result['max_drawdown'] * 100:.2f}%")
    print(f"  夏普比率: {original_result['sharpe_ratio']:.2f}")
    print(f"  胜率: {original_result['win_rate'] * 100:.2f}%")
    print(f"  盈亏比: {original_result['win_loss_ratio']:.2f}")
    
    print("\n新权重 (R:40%, O:25%, E:20%, M:15%):")
    print(f"  总收益率: {new_result['total_return'] * 100:.2f}%")
    print(f"  年化收益率: {new_result['annualized_return'] * 100:.2f}%")
    print(f"  最大回撤: {new_result['max_drawdown'] * 100:.2f}%")
    print(f"  夏普比率: {new_result['sharpe_ratio']:.2f}")
    print(f"  胜率: {new_result['win_rate'] * 100:.2f}%")
    print(f"  盈亏比: {new_result['win_loss_ratio']:.2f}")
...
def analyze_event_signals(data, signals):
    ...
    if trades:
        ...
        print(f"亏损交易次数: {len(loss_trades)}")
        if win_trades:
            print(f"平均盈利: {np.mean([t['return'] for t in win_trades]) * 100:.2f}%")
        if loss_trades:
            print(f"平均亏损: {np.mean([abs(t['return']) for t in loss_trades]) * 100:.2f}%")
...
def main():
    ...
    try:
        ...
        # 生成模拟数据
        data = generate_mock_data(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
        ...
        print(f"盈亏比: {best_params['win_loss_ratio']:.2f}")
        ...
        gram_strategy = GRAMFourFactorStrategy()
        factor_data = gram_strategy.calculate_gram_factors(data)
        best_signals = gram_strategy._generate_signals(
            factor_data, 
            (best_params['batch_buy_min'], best_params['batch_buy_max'])
        )
        ...
        for i in range(1, len(df)):
            ...
            if position != 0:
                if position > 0:
                    ...
                    daily_return = (df['close'].iloc[i-1] - df['close'].iloc[i]) / df['close'].iloc[i-1]
                portfolio_value *= (1 + daily_return * abs(position))
            cumulative_returns.append(portfolio_value)
        
        # 绘制净值曲线
        plt.figure(figsize=(12, 6))
        plt.plot(df.index, cumulative_returns, label='策略净值')
        plt.plot(df.index, (1 + df['close'].pct_change()).cumprod(), label='基准净值')
